...
def cetak_transaksi(databarang):
    ...
    print("+===========================================================+")
    print()
    print_centered(f"{'Total:' :<30}{format_currency(total).rjust(14)}")
...
def pembayaran(subtotal):
    while True:
        try:
            ...
            print_centered(f"{'Total setelah diskon:' :<30}{format_currency(total_setelah_diskon).rjust(14)}")
            print()
            print("-----------------------------------------------------------")
            
            pembayaran = int(input("   Pembayaran-Tunai:        Rp"))

            if pembayaran < total_setelah_diskon:
                print_centered("Jumlah pembayaran kurang dari total yang harus dibayar! Silakan masukkan jumlah yang cukup.")
            else:
                kembalian = pembayaran - total_setelah_diskon
                print_centered(f"{'Kembalian:' :<30}{format_currency(kembalian).rjust(14)}")
...
def menu(): 
    while True:
        ...
        if pilih == "1":
            ...
            print()  # Tambahkan baris kosong setelah mencetak transaksi
        elif pilih == "3":
            total = cetak_transaksi(databarang)
            pembayaran(total)
            print()
